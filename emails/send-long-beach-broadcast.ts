...
/**
 * Send the Long Beach (SuperTri) race-week email to all active subscribers.
 *
 * Usage:
 *   RESEND_API_KEY=re_xxx RESEND_AUDIENCE_ID=xxx npx tsx emails/send-long-beach-broadcast.ts --dry-run
 *
 * Options:
 *   --dry-run     Preview recipients without sending
 *
 * Safety: refuses to send while placeholder data remains in the rendered
 * email. Images must be deployed to patrickwingert.com/email/ before
 * broadcasting (the broadcast uses hosted URLs, not CID attachments).
 */
...
async function main() {
  ...
  const sampleHtml = await render(
    React.createElement(LongBeachEmail, { email: "sample@example.com" }),
  );
  ...
  if (isDryRun) {
    ...
    console.log(`Subject: ${longBeachSubject}`);
  ...
  for (const contact of activeContacts) {
    const rawHtml = await render(
      React.createElement(LongBeachEmail, { email: contact.email }),
    );
