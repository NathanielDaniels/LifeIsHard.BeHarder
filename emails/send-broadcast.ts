...
import SeasonUpdateEmail from "./season-update-email-hybrid";

/**
 * Send the season update email to ALL subscribers via Resend Broadcasts.
 *
 * Usage:
 *   RESEND_API_KEY=re_xxx RESEND_AUDIENCE_ID=xxx npx tsx emails/send-broadcast.ts
 *
 * Options:
...
/**
 * Injects bgcolor HTML attributes for Gmail iOS compatibility.
 * Gmail iOS strips inline background-color on some elements but respects bgcolor.
 */
...
async function main() {
  ...
  console.log(
    `Found ${activeContacts.length} active subscribers (${(contacts?.data || []).length} total)`,
  );
  ...
  if (isDryRun) {
    ...
    console.log(`Subject: The Site Is Updated. The Season Is Loaded. Let's Go.`);
  ...
  // Send individually so each email is rendered with the recipient's address
  console.log("\nSending emails...");
  let sent = 0;
  let failed = 0;
  ...
  for (const contact of activeContacts) {
    const rawHtml = await render(
      React.createElement(SeasonUpdateEmail, { email: contact.email }),
    );
