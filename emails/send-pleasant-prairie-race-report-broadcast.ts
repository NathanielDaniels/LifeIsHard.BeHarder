...
/**
 * Send the Pleasant Prairie race report to all active subscribers.
 *
 * Usage:
 *   RESEND_API_KEY=re_xxx RESEND_AUDIENCE_ID=xxx npx tsx emails/send-pleasant-prairie-race-report-broadcast.ts
 *
 * Options:
...
async function main() {
  ...
  if (isDryRun) {
    ...
    console.log(`Subject: ${pleasantPrairieRaceReportSubject}`);
  ...
  for (const contact of activeContacts) {
    ...
    const { error } = await resend.emails.send({
      ...
      to: contact.email,
      subject: pleasantPrairieRaceReportSubject,
      html,
    });
