...
/**
 * Send the Nationals pre-race email to all active subscribers.
 *
 * Usage:
 *   RESEND_API_KEY=re_xxx RESEND_AUDIENCE_ID=xxx npx tsx emails/send-nationals-broadcast.ts
 *
 * Options:
 *   --dry-run     Preview recipients and run preflight without sending
 *
 * Safety:
 *   1. Refuses to send while placeholder data ("TODO") remains.
 *   2. Refuses to send unless every image URL in the email is live on
 *      patrickwingert.com. The demo script hides missing assets by inlining
 *      them as CID attachments, so a good-looking test is not proof.
 */
...
async function main() {
  ...
  if (isDryRun) {
    ...
    console.log(`Subject: ${nationalsPreRaceSubject}`);
  ...
  for (const contact of activeContacts) {
    ...
    const { error } = await resend.emails.send({
      ...
      to: contact.email,
      subject: nationalsPreRaceSubject,
      html,
    });
