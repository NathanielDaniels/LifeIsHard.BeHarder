...
export async function getSleepHistory(accessToken: string, limit = 7): Promise<WhoopSleep[]> {
  const response = await whoopFetch<{ records: WhoopSleep[] }>(
...
function calculateCurrentHeartRate(
  ...
  const currentHR = Math.round(
    workoutHR - (workoutHR - resting) * easedProgress,
  );
 ...
 */
export async function verifyTokenHealth(
  accessToken: string,
  ...
  const results = await Promise.all(
    (Object.entries(endpoints) as [keyof typeof endpoints, string][]).map(
      async ([name, path]): Promise<
        [keyof typeof endpoints, EndpointHealthResult]
      > => {
        try {
          ...
          console.warn(
            `[whoop-health] ${name}: NETWORK ERROR`,
  ...
  const report = Object.fromEntries(results) as Record<
